local utils = require("heirline.utils")

return function()
    return {
        bright_bg = utils.get_highlight("Folded").bg,
        bright_fg = utils.get_highlight("Folded").fg,
        red = utils.get_highlight("DiagnosticError").fg,
        dark_red = utils.get_highlight("DiffDelete").bg,
        green = utils.get_highlight("String").fg,
        blue = utils.get_highlight("Function").fg,
        gray = utils.get_highlight("NonText").fg,
        orange = utils.get_highlight("Constant").fg,
        purple = utils.get_highlight("Statement").fg,
        cyan = utils.get_highlight("Special").fg,
        diag_warn = utils.get_highlight("DiagnosticWarn").fg,
        diag_error = utils.get_highlight("DiagnosticError").fg,
        diag_hint = utils.get_highlight("DiagnosticHint").fg,
        diag_info = utils.get_highlight("DiagnosticInfo").fg,
        git_del = utils.get_highlight("Removed").fg,
        git_add = utils.get_highlight("Changed").fg,
        git_change = utils.get_highlight("Changed").fg,
        status_line_fg = utils.get_highlight("StatusLine").fg,
        status_line_bg = utils.get_highlight("StatusLine").bg,
        status_line_inactive_fg = utils.get_highlight("StatusLineNC").fg,
        status_line_inactive_bg = utils.get_highlight("StatusLineNC").bg,
        indent_blankline = utils.get_highlight("Character").fg,
        tabline_active_fg = utils.get_highlight("Number").fg,
        tabline_inactive_fg =  utils.get_highlight("Conceal").fg or utils.get_highlight("FoldColumn").fg,
    }
end
